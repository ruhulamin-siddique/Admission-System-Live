...
class UserSecurityManagementTests(TestCase):
    ...
    def test_regular_authenticated_user_can_open_and_update_own_profile(self):
        ...
        self.assertContains(response, 'Edit Identity Details')
...
class AccessRegistryIntegrityTests(TestCase):
    def test_require_access_decorators_reference_registered_permissions(self):
        valid_permissions = {
            (module, task)
            for module, module_info in ACCESS_REGISTRY.items()
        ...
        for path in Path('.').rglob('*.py'):
            ...
            for node in ast.walk(tree):
                ...
                for decorator in node.decorator_list:
                    ...
                    if not call or getattr(call.func, 'id', None) != 'require_access':
                        continue
                    if len(call.args) < 2:
                        continue
                    if not all(isinstance(arg, ast.Constant) and isinstance(arg.value, str) for arg in call.args[:2]):
                        continue
                    permission = (call.args[0].value, call.args[1].value)
                    if permission not in valid_permissions:
                        missing_permissions.append(f'{path}:{node.lineno} {node.name} -> {permission}')
