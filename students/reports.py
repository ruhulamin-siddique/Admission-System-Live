...
def get_academic_analytics(year=None, program=None, batch=None):
    ...
    if year: queryset = queryset.filter(admission_year=year)
    if program: queryset = queryset.filter(program=program)
...
def get_research_demographics(year=None, batch=None, program=None):
    ...
    waiver_stats = queryset.aggregate(
        ...
        full_waiver=Count(Case(When(waiver__gte=100, then=1), output_field=IntegerField())),
        partial_waiver=Count(Case(When(waiver__gt=0, waiver__lt=100, then=1), output_field=IntegerField())),
